...
GIT_COIN_HASH=`git ls-tree HEAD src/3rdParty/coin/ | awk '{print $3}'`
GIT_PIVY_HASH=`git ls-tree HEAD src/3rdParty/pivy/ | awk '{print $3}'`
...
echo "GIT_COIN_HASH= ${GIT_COIN_HASH}"   >> ${CURDIR}/Makefile.git_rev
echo "GIT_PIVY_HASH= ${GIT_PIVY_HASH}"   >> ${CURDIR}/Makefile.git_rev
