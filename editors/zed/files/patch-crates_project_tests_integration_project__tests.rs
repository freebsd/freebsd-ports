--- crates/project/tests/integration/project_tests.rs.orig	2026-08-26 14:24:53 UTC
...
+#[cfg(any(target_os = "linux", target_os = "freebsd"))]
 ...
 #[cfg(not(windows))]
 use std::os;
@@ -6711,7 +6711,7 @@ async fn test_rescan_and_remote_updates(cx: &mut gpui:
     });
 }
...
+#[cfg(any(target_os = "linux", target_os = "freebsd"))]
 ...
 async fn test_recreated_directory_receives_child_events(cx: &mut gpui::TestAppContext) {
     init_test(cx);
@@ -13714,7 +13714,7 @@ async fn test_git_repository_status_removes_directory_
     });
 }
...
+#[cfg(any(target_os = "linux", target_os = "freebsd"))]
 ...
 async fn test_git_events_after_project_excludes_dot_git(cx: &mut gpui::TestAppContext) {
     init_test(cx);
@@ -16512,7 +16512,7 @@ fn git_reset(offset: usize, work_dir: &Path) {
     );
 }
...
@@ -16527,7 +16527,7 @@ fn git_branch(name: &str, work_dir: &Path) {
     );
 }
