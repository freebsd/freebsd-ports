--- crates/project/tests/integration/project_tests.rs.orig	2026-09-09 15:43:20 UTC
...
+#[cfg(any(target_os = "linux", target_os = "freebsd"))]
 ...
 #[cfg(not(windows))]
 use std::os;
@@ -7013,7 +7013,7 @@ async fn test_rescan_and_remote_updates(cx: &mut gpui:
     });
 }
...
+#[cfg(any(target_os = "linux", target_os = "freebsd"))]
 ...
 async fn test_recreated_directory_receives_child_events(cx: &mut gpui::TestAppContext) {
     init_test(cx);
@@ -14016,7 +14016,7 @@ async fn test_git_repository_status_removes_directory_
     });
 }
...
+#[cfg(any(target_os = "linux", target_os = "freebsd"))]
 ...
 async fn test_git_events_after_project_excludes_dot_git(cx: &mut gpui::TestAppContext) {
     init_test(cx);
@@ -16814,7 +16814,7 @@ fn git_reset(offset: usize, work_dir: &Path) {
     );
 }
...
@@ -16829,7 +16829,7 @@ fn git_branch(name: &str, work_dir: &Path) {
     );
 }
