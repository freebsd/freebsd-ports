Revert upstream commit a6bb693.
That commit removed conditional definitions for some constants and
enforced them to haved the same values than on Linux. But we are on
FreeBSD: ABI is different.

See https://bugs.freebsd.org/bugzilla/show_bug.cgi?id=294062 for details.

--- libgcc/config/aarch64/cpuinfo.c.orig	2026-05-03 15:39:45 UTC
+++ libgcc/config/aarch64/cpuinfo.c
@@ -47,61 +47,208 @@ typedef struct __ifunc_arg_t {
...
@@ -258,4 +405,5 @@ __init_cpu_features(void)
 #undef getCPUFeature
 #undef setCPUFeature
 }
