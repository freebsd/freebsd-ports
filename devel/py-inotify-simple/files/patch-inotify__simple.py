...
+++ inotify_simple.py
@@ -3,6 +3,7 @@ from select import poll
...
@@ -76,12 +77,19 @@ class INotify(FileIO):
...
+        if _libc is None:
+            _uname = os.uname()
+            if _uname.sysname == 'FreeBSD' and \
+                    int(_uname.release.split('.')[0]) < 15:
+                _libc = CDLL(find_library('inotify'), use_errno=True)
+            else:
