--- chrome/browser/ui/webui/management/management_ui_handler.cc.orig	2026-08-13 07:41:05 UTC
...
@@ -196,7 +196,7 @@ void AddThreatProtectionPermission(const char* title,
...
@@ -397,7 +397,7 @@ void ManagementUIHandler::AddBrowserReportingInfo(
...
@@ -429,7 +429,7 @@ void ManagementUIHandler::AddProfileReportingInfo(
     report_sources->Append(std::move(data));
   }
...
+#if BUILDFLAG(IS_WIN) || BUILDFLAG(IS_MAC) || BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_BSD)
   ...
   if (!device_signal_data.empty()) {
     report_sources->Append(std::move(device_signal_data));
@@ -619,7 +619,7 @@ policy::PolicyService* ManagementUIHandler::GetPolicyS
       ->policy_service();
 }
