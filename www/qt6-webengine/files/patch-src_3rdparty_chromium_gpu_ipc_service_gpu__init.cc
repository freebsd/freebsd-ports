--- src/3rdparty/chromium/gpu/ipc/service/gpu_init.cc.orig	2026-02-26 14:39:03 UTC
+++ src/3rdparty/chromium/gpu/ipc/service/gpu_init.cc
@@ -157,7 +157,7 @@ bool CanAccessDeviceFile(const GPUInfo& gpu_info) {
...
+#if BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS) || BUILDFLAG(IS_BSD)
         VLOG(1) << "Quit GPU process launch to fallback to SwiftShader cleanly "
                 << "on Linux";
         return false;
@@ -792,7 +792,7 @@ bool GpuInit::InitializeAndStartSandbox(base::CommandL
...
@@ -844,7 +844,7 @@ bool GpuInit::InitializeAndStartSandbox(base::CommandL
       }
     }
   }
-#if BUILDFLAG(IS_LINUX) || \
...
+#if BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_BSD) || \
   ...
   if (!gl_disabled && !gl_use_swiftshader_ && std::getenv("RUNNING_UNDER_RR")) {
     // https://rr-project.org/ is a Linux-only record-and-replay debugger that
@@ -1111,7 +1111,7 @@ void GpuInit::InitializeInProcess(base::CommandLine* c
     }
   }
